...
sns.barplot(x='LLM', y='Accuratezza', data=df, ci=None, label='Accuratezza', color='blue', alpha=0.7)
sns.barplot(x='LLM', y='Completezza', data=df, ci=None, label='Completezza', color='green', alpha=0.5)

# Aggiungi informazioni al grafico
plt.title("Accuratezza e Completezza per LLM", fontsize=16)
plt.ylabel("Valore")
plt.xlabel("LLM")
plt.legend(["Accuratezza", "Completezza"])
